...
//
//        if (pipeId == -1) {
//            // Если не нашли подходящую трубу, предлагаем создать новую
//            std::cout << "Нет свободных труб с диаметром " << desiredDiameter << " мм. Создать новую трубу? (1 - Да, 0 - Нет): ";
//            int choice = inputInRange<int>("", 0, 1);
//            if (choice == 1) {
//                // Переходим в меню создания новой трубы
//                std::cout << "Создание новой трубы.\n";
//                Pipe::addPipe();
//                // Проверяем, была ли создана труба с нужным диаметром
//                bool newPipeFound = false;
//                for (const auto& [id, pipe] : Pipe::pipes) {
//                    // Проверяем, что труба имеет нужный диаметр и не используется в соединении
//                    bool pipeInUse = false;
//                    for (const auto& [connId, conn] : connections) {
...
//                }
//                if (!newPipeFound) {
//                    std::cout << "Созданная труба не имеет требуемый диаметр. Попробуйте снова.\n";
//                    logger.log("Созданная труба не имеет требуемый диаметр.");
//                    continue;
//                }
//            } else {
